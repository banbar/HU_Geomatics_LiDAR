...
def return_image_time(data, image_index):
    
    for i in range(len(data)):
        # print(i)
        # print(data[i]['time'])
        l4 = data[i]['filePath'].rstrip('\n').split('\\')[-1].split('.')[0][-1]
        l3 = data[i]['filePath'].rstrip('\n').split('\\')[-1].split('.')[0][-2]
        l2 = data[i]['filePath'].rstrip('\n').split('\\')[-1].split('.')[0][-3]
        l1 = data[i]['filePath'].rstrip('\n').split('\\')[-1].split('.')[0][-4]
        image_num = l1 + l2 + l3 + l4
        # print(int(image_num))
        if(int(image_num) == image_index):
            return data[i]['time']
